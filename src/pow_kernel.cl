           ...
           __private size_t gr_id)
{
    ...
    *nonce_probe = HIGH_BITS;
    for (i = HASH_TRITS_LENGTH - *min_weight_magnitude; i < HASH_TRITS_LENGTH;
         i++) {
                     ...
                     __constant size_t *loop_count)
{
    ...
    for (i = 0; i < *loop_count; i++) {
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
        if (id == 0)
            check(&(state_low[gid]), &(state_high[gid]), found,
                  min_weight_magnitude, &(nonce_probe[gr_id]), gr_id);
            ...
            if (j < HASH_TRITS_LENGTH) {
                trit_hash[j] =
                    (mid_low[gid + j] & nonce_probe[gr_id]) == 0
                        ? 1
                        : (mid_high[gid + j] & nonce_probe[gr_id]) == 0 ? -1
                                                                        : 0;
            }
        }
    }
